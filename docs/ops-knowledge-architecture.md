...
| `doc_type` | 文档类型 | `fault` / `sop` / `emergency` / `solution` / `event` |
...
| `doc_type` | TEXT NOT NULL | 文档类型（fault/sop/emergency/solution/event） |
...
| **SOP** (sop) | 按章节/步骤分割，metadata标记步骤序号 | 用户可能查整个SOP流程，也可能只查某个具体操作步骤 |
| **应急预案** (emergency) | 整个预案作为一个chunk | 应急场景下不能接受信息碎片化，必须返回完整预案 |
...
| `doc_type` | string | 否 | 过滤文档类型（fault/sop/emergency/solution/event） |
...
sequenceDiagram
    ...
    Tool->>RAG: 语义搜索 + metadata过滤
    RAG-->>Tool: 匹配的chunks + scores
    Tool-->>Agent: 检索结果列表
```
...
sequenceDiagram
    ...
    Search->>RAG: 语义搜索 + metadata过滤
    RAG-->>Search: 匹配的故障案例chunks
    Search-->>Agent: 检索结果
...
sequenceDiagram
    ...
    Agent->>Search: query="F5负载均衡应急预案",<br/>doc_type="emergency",<br/>device_vendor="f5",<br/>device_type="loadbalancer"
    Search->>RAG: 语义搜索 + metadata过滤
    Note over RAG: emergency类型<br/>整个预案作为单个chunk
    RAG-->>Search: 完整预案内容
    Search-->>Agent: 检索结果
